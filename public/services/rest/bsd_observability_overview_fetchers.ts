...
import { callBSDApi } from './create_call_bsd_api';
...
export const fetchObservabilityOverviewPageData = async ({
  ...
  const data = await callBSDApi('GET /internal/apm/observability_overview', {
    signal: null,
    params: {
      query: {
  ...
  return {
    appLink: `/app/bsd/services?rangeFrom=${relativeTime.start}&rangeTo=${relativeTime.end}`,
    stats: {
      services: {
        type: 'number',
        value: serviceCount,
...
export async function getHasData() {
  return await callBSDApi('GET /internal/apm/observability_overview/has_data', {
    signal: null,
  });
}
