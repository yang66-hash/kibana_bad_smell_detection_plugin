...
import { APIReturnType, callBSDApi } from '../../../../services/rest/create_call_bsd_api';
...
async function saveApmIndices({ apmIndices }: { apmIndices: Record<string, string> }) {
  await callBSDApi('POST /internal/apm/settings/apm-indices/save', {
    signal: null,
    params: {
      body: apmIndices,
    },
...
export function ApmIndices() {
  ...
  const { data = INITIAL_STATE, refetch } = useFetcher((_callBSDApi) => {
    return _callBSDApi(`GET /internal/apm/settings/apm-index-settings`);
  }, []);
