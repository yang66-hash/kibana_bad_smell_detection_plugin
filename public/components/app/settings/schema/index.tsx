...
import { callBSDApi, APIReturnType } from '../../../../services/rest/create_call_bsd_api';
...
export function Schema() {
  ...
  const { toasts } = useBSDPluginContext().core.notifications;
...
async function getUnsupportedApmServerConfigs(toasts: NotificationsStart['toasts']) {
  try {
    const { unsupported } = await callBSDApi(
...
async function createCloudApmPackagePolicy(
  ...
  try {
    const { cloudApmPackagePolicy } = await callBSDApi(
