...
type DependencyReturn = APIReturnType<'GET /internal/bsd/service-map/dependency'>;
...
export function DependencyContents({ elementData, environment, start, end }: ContentsProps) {
  ...
  const { data = INITIAL_STATE, status } = useFetcher(
    (callApmApi) => {
      if (dependencyName) {
        return callApmApi('GET /internal/bsd/service-map/dependency', {
          params: {
            query: {
              dependencyName,
              environment,
  ...
  const detailsUrl = dependencyName
    ? apmRouter.link('/dependencies/overview', {
        query: {
          ...query,
          dependencyName,
        } as TypeOf<BSDRoutes, '/dependencies/overview'>['query'],
      })
    : undefined;
