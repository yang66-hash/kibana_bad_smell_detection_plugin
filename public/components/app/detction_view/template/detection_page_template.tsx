...
export function DetectionPageTemplate({
  introductionText
}:DetectionPageProps) {
  ...
  const getBSDataByType = async () => {
    try {
      ...
      const response = await http?.get(`/api/bsd/bs_by_type/${badSmellType}`);
  ...
  useEffect(()=>{
    ...
    serviceSelectedItems.forEach((service) => {
      badSmellSelectedItems.forEach((badSmell) => {
        newURLArr.push([backendPoints + "/" + getBadSmellPrefixByName(badSmellType) + "/" + dealWithBadSmellName(badSmell.badSmellName), service.serviceName, badSmell.badSmellName]); 
      });
    });
  ...
  const onClickToStartDetection = () =>{
    console.log("start detection in " + badSmellType);
