    ...
    (callBSDApi) => {
      if (preferred) {
        return callBSDApi('GET /internal/apm/services', {
          ...
          const filteredStatisticsItem = mainStatisticsData.items.filter(data=>data.agentName === 'java' || data.agentName === 'opentelemetry/java');
          return {
            requestId: uuidv4(),
            ...mainStatisticsData,
            items:filteredStatisticsItem,
            serviceOverflowCount: filteredStatisticsItem>1000? filteredStatisticsItem - 1000:0,
            maxCountExceeded: filteredStatisticsItem>1000?true:false,
          };
        });
      }
    },
...
export function ServiceInventory() {
  //use for debouncing of search query, default time 300ms
  ...
  return (
    <>
      ...
      <div data-fastSearch={isTableSearchBarEnabled ? 'enabled' : 'disabled'}>
        <SearchBar showTimeComparison  searchBarPlaceholder='filter services here(support KQL)'/>
      </div>
