...
import { EVENT_OUTCOME } from '../../../../common/es_fields/bsd';
...
export function useFailedTransactionsCorrelations() {
  ...
  const startFetch = useCallback(async () => {
    ...
    try {
      ...
      const overallHistogramResponse = await callBSDApi(
      ...
      const errorHistogramResponse = await callBSDApi(
        'POST /internal/apm/latency/overall_distribution/transactions',
        {
          signal: abortCtrl.current.signal,
          params: {
      ...
      const { fieldCandidates: candidates } = await callBSDApi(
      ...
      for (const fieldCandidatesChunk of fieldCandidatesChunks) {
        const pValues = await callBSDApi('POST /internal/apm/correlations/p_values/transactions', {
          signal: abortCtrl.current.signal,
          params: {
            body: {
              ...fetchParams,
