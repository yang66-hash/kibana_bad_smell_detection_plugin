  ...
  const startFetch = useCallback(async () => {
    ...
    try {
      ...
      const {
        ...
        durationMin,
        durationMax,
      } = await callBSDApi('POST /internal/apm/latency/overall_distribution/transactions', {
      ...
      const { fieldCandidates } = await callBSDApi(
      ...
      for (const fieldCandidateChunk of fieldCandidateChunks) {
        const fieldValuePairChunkResponse = await callBSDApi(
      ...
      for (const fieldValuePairChunk of fieldValuePairChunks) {
        const significantCorrelations = await callBSDApi(
