...
import { useAdHocBSDDataView } from '../../../hooks/use_adhoc_apm_data_view';
...
function ActionMenuSections({
  ...
  const { core, uiActions, share } = useBSDPluginContext();
  const location = useLocation();
  const apmRouter = useBSDRouter();
  const { dataView } = useAdHocBSDDataView();
  ...
  const infraLinksAvailable = useBSDFeatureFlag(ApmFeatureFlagName.InfraUiAvailable);
