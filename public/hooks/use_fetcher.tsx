...
import { AutoAbortedBSDClient, callBSDApi } from '../services/rest/create_call_bsd_api';
...
const createAutoAbortedBSDClient = (
  signal: AbortSignal,
  addInspectorRequest: <Data>(result: FetcherResult<Data>) => void
): AutoAbortedBSDClient => {
  return ((endpoint, options) => {
    return callBSDApi(endpoint, {
      ...options,
      signal,
    } as any)
      .catch((err) => {
...
export function useFetcher<TReturn>(
  fn: (callBSDApi: AutoAbortedBSDClient) => TReturn,
  fnDeps: any[],
  options: {
  ...
  useEffect(() => {
    ...
    async function doFetch() {
      ...
      const promise = fn(createAutoAbortedBSDClient(signal, addInspectorRequest));
