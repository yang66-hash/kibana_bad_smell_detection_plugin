...
// plugin config
export const config: PluginConfigDescriptor<BSDConfig> = {
...
export type BSDConfig = TypeOf<typeof configSchema>;

export const plugin = async (initContext: PluginInitializerContext) => {
  const { BSDPlugin } = await import('./plugin');
  return new BSDPlugin(initContext);
};

export { APM_SERVER_FEATURE_ID } from '../common/rules/apm_rule_types';
export { BSDPlugin } from './plugin';
export type { BSDPluginSetup } from './types';
export type {
