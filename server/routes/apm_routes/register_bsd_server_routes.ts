...
import type { BSDConfig } from '../..';
import type { BSDPluginSetupDependencies, BSDPluginStartDependencies } from '../../types';
  ...
  routes.forEach((route) => {
    ...
    if (!version) {
      ...
      (
        ...
        path: pathname,
        access: pathname.includes('/internal/bsd') ? 'internal' : 'public',
        options,
      }).addVersion(
        {
          version,
...
type Plugins = {
  [key in keyof BSDPluginSetupDependencies]: {
    setup: Required<BSDPluginSetupDependencies>[key];
    start: () => Promise<Required<BSDPluginStartDependencies>[key]>;
  };
};
